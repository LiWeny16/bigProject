...
const validateFunction_login = (req, res, next) => {
    const reg_login_schema = Joi.object({  //验证规则
        ...
        password: Joi.string().required().regex(/^[a-zA-Z0-9]{3,18}$/).error(new Error(`
        {
            status:0,
            msg:'密码不合法，宝'
        }`
        ...
        email: Joi.string().email().error(new Error(`
        {
            status:0,
            msg:'邮箱不合法，宝'
        }`
...
module.exports = (validateFunction_login)
